...
float calculateAxisPID(float error, float& integral, float& prev_error, float& prev_derivative, float& prev_output, float kp, float ki, float kd, float dt, bool first_run);
...
void updateStabilizer() {
    ...
    if (current_control_mode == MODE_STABILIZED) {
        ...
        float targetRoll = inputY * 45.0f;

        //DEBUG_PRINTF("Target Pitch: %.2f, Target Roll: %.2f\n", targetPitch, targetRoll);

        // --- PITCH AXIS (SINGLE LOOP CONTROLLER) ---
        // Find the error between the desired pitch and the actual main body pitch
        float pError = targetPitch - mPitch;
        if (abs(pError) < robotSettings.deadband) {
            pError = 0.0f;
        }
        
        // Calculate the motor output directly using the standard pitch PID settings
        float outX = calculateAxisPID(pError, pitch_inner_integral, pitch_inner_prev_error, 
                                      pitch_inner_prev_derivative, pitch_inner_prev_output, 
                                      robotSettings.kp_pitch, robotSettings.ki_pitch, robotSettings.kd_pitch, 
                                      dt, pid_first_run);
        outX *= robotSettings.pitch_dir; // Apply pitch direction multiplier
        ...
        float outY = calculateAxisPID(rInnerError, roll_inner_integral, roll_inner_prev_error, roll_inner_prev_derivative, roll_inner_prev_output, 
                                        ...
                                        dt, pid_first_run);
        outY *= robotSettings.roll_dir; // Apply roll direction multiplier
