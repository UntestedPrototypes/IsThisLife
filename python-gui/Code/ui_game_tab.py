...
class GameControllerTab:
    ...
    def _setup_ui(self):
        ...
        # CHANGED: Start with a default [1], but it will be updated dynamically
        self.robot_combo = ttk.Combobox(sel_frame, textvariable=self.robot_var, width=5)
        self.robot_combo['values'] = [1] 
        ...
        btn_apply = ttk.Button(edit_frame, text="Apply Settings to Robot", command=self._assign_controller)
        btn_apply.pack(fill=tk.X, padx=20, pady=(10, 10))

    def update_available_robots(self, robot_ids):
        """Called by Dashboard when new robots are detected"""
        current_selection = self.robot_combo.get()
        
        # Combine detected IDs with any currently assigned IDs (so we don't hide assigned ones)
        assigned_ids = list(self.assignments.keys())
        all_ids = sorted(list(set(robot_ids + assigned_ids + [1]))) # Always keep 1 as fallback
        
        self.robot_combo['values'] = all_ids
        
        # If the box was empty or invalid, select the first available
        if not current_selection and all_ids:
            ...
            try:
                ...
                if isinstance(val, dict):
                    self.assignments[r_id] = {
                        'id': None, 'guid': val.get('guid', ''), 'name': "Searching...",
                        'scales': val.get('scales', default_scales.copy())
                    }
                else:
                    self.assignments[r_id] = {
                        'id': None, 'guid': str(val), 'name': "Searching...",
    ...
    def get_assignment_guids(self):
        ...
        for r_id, info in self.assignments.items():
            data[str(r_id)] = {'guid': info['guid'], 'scales': info['scales']}
        return data
        ...
        try:
            ...
            if r_id in self.assignments:
                ...
                for key, val in scales.items():
                    if key in self.learning_button_map and self.learning_button_map[key]["scale_sb"]:
    ...
    def _assign_controller(self):
        try:
            ...
            if not c_name and r_id in self.assignments:
                ctrl_id, ctrl_guid, ctrl_name = self.assignments[r_id]['id'], self.assignments[r_id]['guid'], self.assignments[r_id]['name']
            elif c_name in self.controller_map:
                ctrl = self.controller_map[c_name]
                ctrl_id, ctrl_guid, ctrl_name = ctrl.id, ctrl.guid, ctrl.name
            else:
            ...
            for existing_r, info in list(self.assignments.items()):
                if info['id'] == ctrl_id and existing_r != r_id and ctrl_id is not None: del self.assignments[existing_r]
            self.assignments[r_id] = {'id': ctrl_id, 'guid': ctrl_guid, 'name': ctrl_name, 'scales': scales}
    ...
    def _update_assignment_list(self):
        ...
        for r_id, info in self.assignments.items():
            c_id = info['id']
            is_connected = (c_id is not None and self.manager.get_controller(c_id))
            ...
            sc = info['scales']
            self.tree.insert("", "end", values=(f"Robot {r_id}", display_name, c_id if c_id is not None else "-", f"F:{sc['vx']}% L:{sc['vy']}% R:{sc['omega']}%"))
    ...
    def refresh_controllers(self):
        ...
        self._update_assignment_list()
        if values and self.controller_combo.get() not in values:
